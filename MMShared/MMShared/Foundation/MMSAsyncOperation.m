...
#import "MMSAsyncOperation.h"
#import "MMSError.h"
#import "NSError+MMS.h"

@interface MMSAsyncOperation()
...
@implementation MMSAsyncOperation
...
- (void)start{
    ...
    if(self.isCancelled){
        ...
        NSError* error = [NSError mms_errorWithDomain:MMSharedErrorDomain code:MMSErrorOperationCancelled descriptionFormat:@"The %@ was cancelled before it started", self.class];
...
- (void)cancel{
    [super cancel];
    NSError* error = [NSError mms_errorWithDomain:MMSharedErrorDomain code:MMSErrorOperationCancelled descriptionFormat:@"The %@ was cancelled", self.class];
    [self finishWithError:error];
}
