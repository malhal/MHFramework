...
- (instancetype)initWithRecordName:(NSString *)recordName{
    ...
    if (self) {
        ...
        self.databaseScope = MMSDatabaseScopeFromString(scopeString);
...
- (NSString *)recordName{
    return [NSString stringWithFormat:@"%@::%@/%@::%@::%@::%@", self.recordType, self.userSpecificOwnerName, MMSDatabaseScopeString(self.databaseScope), self.zoneName, self.shareOwnerName, self.sharedRecordName];
}
