...
#import "MMSBigSplitViewController.h"
#import "UIViewController+MMS.h"
//#import "MMSDetailItemSplitter.h"
#import "UIView+MMS.h"
...
@interface MMSBigSplitViewController()

//@property (copy, nonatomic) UITraitCollection *forcedTraitCollection;

// used for the delegates when is the child split.
//@property (strong, nonatomic, readwrite) MMSMasterItemSplitter *masterItemSplitter;

//@property (strong, nonatomic) MMSRootMasterSplitViewController *rootMasterSplitViewController;
...
@implementation MMSBigSplitViewController
//@dynamic masterViewController; // private
/*
- (void)showDetailViewController:(UIViewController *)vc sender:(id)sender{
    [super showDetailViewController:vc sender:sender];
    //self.currentSplitDetailItem = vc.mms_splitDetailItem;
}
...
- (void)viewDidLoad {
    ...
    MMSSplitViewController* childSplitController = self.childSplitController;
    if(childSplitController){
       // childSplitController.delegate = self.masterItemSplitter;
        //self.detailItemSplitter = [MMSDetailItemSplitter.alloc initWithSplitViewController:childSplitController];
...
- (MMSSplitViewController *)childSplitController{
    id i = self.viewControllers.firstObject;
    //return i; // sometimes is a nav controller
    return MMSDynamicCast(MMSSplitViewController.class, i);
}
...
//    id detailItem = self.mms_viewedObject;
//    if(!detailItem){
//        // If our secondary controller doesn't show a detail item, do the collapse ourself by doing nothing
//        return YES;
//    }
//    else if (![secondaryViewController mms_containsViewedObject:detailItem]) {
//        return YES;
//    }
//    return NO;
//}
...
//            UIViewController *vcc = view.mms_viewController; // RootViewController or MasterViewController
...
- (UIViewController *)childContainingSender:(id)sender{
    ...
    BOOL b = [sender mms_isMemberOfViewControllerHierarchy:vc];
    if(b){
        return vc;
    }
    vc = self.masterViewController;
    b = [sender mms_isMemberOfViewControllerHierarchy:vc];
    if(b){
        return vc;
    }
    return nil;
...
@implementation UIViewController (MMSBigSplitViewController)

- (MMSBigSplitViewController *)mms_bigSplitViewController{
    UISplitViewController *svc = self.splitViewController;
    if([svc isKindOfClass:MMSBigSplitViewController.class]){
        return (MMSBigSplitViewController *)svc;
    }
    return svc.mms_bigSplitViewController;
}

//- (void)mms_setSplitDetailItem:(id)splitDetailItem {
//
//}
