...
//In code change MasterViewController to be a subclass of MMSMapViewController and delete all table code.
//That's it!
...
// Choose 'annotation detail' segue and set the identifer showAnnotation, class MMSAnnotationSegue, kind Custom.
...
//typedef NS_ENUM(NSInteger, MMSAnnotationsTablePresentationStyle) {
//    MMSAnnotationsTablePresentationStyleModal,
//    MMSAnnotationsTablePresentationStyleSheet
//};

//MMS_EXTERN NSString * const MMSShowAnnotationSegueIdentifier; // The default is 'showAnnotation' so set that identifier in the storyboard manual segue.
//MMS_EXTERN NSString * const MMSAnnotationCellIdentifier; // To use a custom cell in the table view that is hooked up to the outlet use the identifier 'annotation'

@class MMSMapTypeBarButtonItem;

@interface MMSMapViewController : UIViewController<MKMapViewDelegate>//, UITableViewDataSource, UITableViewDelegate>
...
@property (nonatomic, strong, readonly) MMSMapTypeBarButtonItem *mapTypeBarButtonItem;
...
//@property (nonatomic, assign) MMSannotationsTablePresentationStyle annotationsTablePresentationStyle;
