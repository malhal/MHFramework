//
//  MMSMasterTableViewController.m
//  MMShared
//
...
#import "MMSMasterTableViewController.h"
#import "UINavigationController+MMS.h"
#import "UIViewController+MMS.h"

@interface MMSMasterTableViewController ()

@end

@implementation MMSMasterTableViewController
...
//    }
   ...
   // [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(navigationControllerWillShowViewController:) name:MMSNavigationControllerWillShowViewControllerNotification object:nil];
...
//        return;
//    }
//    UIViewController *last = notification.userInfo[MMSNavigationControllerLastVisibleViewController];
//    if(!last){
//        return;
//    }
    //if(last == self.detailViewController){
//        if(next != )
     //   UIViewController *next = notification.userInfo[MMSNavigationControllerNextVisibleViewController];
...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    //[NSNotificationCenter.defaultCenter removeObserver:self name:MMSNavigationControllerWillShowViewControllerNotification object:nil];
...
//    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(navigationControllerWillShowViewController:) name:MMSNavigationControllerDidShowViewControllerNotification object:detailViewController.navigationController];
//
//}
...
//    BOOL editButtonItemEnabled = NO;
    ...
    //if(detailNav.mms_isViewVisible){
   ...
   // }
    ...
    //                NSIndexPath *indexPath = [dataSource indexPathForElementWithModelIdentifier:splitItem inView:self.tableView];
        ...
        //CGRect safeArea = UIEdgeInsetsInsetRect(self.tableView.bounds, self.tableView.safeAreaInsets);
          ...
          //  NSArray *rows = self.tableView.mms_indexPathsForSafeAreaRows;
...
@implementation UIViewController (MMSMasterViewController)

- (NSObject *)mms_currentDetailViewControllerWithSender:(id)sender{
    UIViewController *target = [self targetViewControllerForAction:@selector(mms_currentDetailViewControllerWithSender:) sender:sender];
    if(target){
        return [target mms_currentDetailViewControllerWithSender:sender];
    }else{
        return nil;
    }
}
